//
//  SavingsCard.swift
//  Snusless
//
//  Created by Aurelie Vaudan APP25 on 2026-05-22.
//

import SwiftUI

struct SavingsCard: View {
    var user: User
    
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 20)
                .fill(.lightGreen)
            
            Circle()
                .stroke(
                    .white.opacity(0.2),
                    style: StrokeStyle(lineWidth: 10, dash: [100, 100])
                )
                .frame(width: 180, height: 180)
                .position(x: 75, y: 142)
            
            VStack {
                VStack {
                    Text("Längsta streak:")
                        .font(.custom("Afacad-Regular", size: 16))
                        .foregroundColor(.white)
                    Text(
                        user.longestStreak > 0
                        ? LocalizedStringKey("\(user.longestStreak) dagar")
                        : LocalizedStringKey("Ingen streak hittad")
                    )
                    .font(.custom("Afacad-Bold", size: 18))
                    .foregroundColor(
                        user.longestStreak > 0 ? .darkGreen : .black.opacity(0.3)
                    )
                }
                .padding(.top, 10)
                
                Spacer()
                
                HStack {
                    VStack(alignment: .leading, spacing: -10) {
                        HStack(alignment: .firstTextBaseline, spacing: 8) {
                            Text("\(user.streak.currentStreak)")
                                .font(.custom("Afacad-Bold", size: 40))
                                .foregroundColor(.white)
                            
                            Text("dagar")
                                .font(.custom("Afacad-Regular", size: 20))
                                .foregroundColor(.white)
                        }
                        
                        Text("utan snus")
                            .font(.custom("Afacad-Regular", size: 20))
                            .foregroundColor(.white)
                    }
                    
                    Spacer()
                    
                    VStack(alignment: .leading, spacing: -8) {
                        Text("Du har sparat")
                            .font(.custom("Afacad-Regular", size: 20))
                            .foregroundColor(.white)
                        
                        HStack(alignment: .firstTextBaseline, spacing: 2) {
                            Text(String(format: "%.0f", user.totalSaved))
                                .font(.custom("Afacad-Bold", size: 40))
                                .foregroundColor(.white)
                            
                            Text("kr")
                                .font(.custom("Afacad-Regular", size: 24))
                                .foregroundColor(.white)
                            
                        }
                    }
                }
                .padding(.horizontal, 30)
                
                Spacer()
                
                CustomProgressView(user: user)
            }
        }
        .clipped()
        .frame(height: 220)
        .frame(width: 380)
        .padding(.horizontal, 20)
        .shadow(color: .black.opacity(0.4), radius: 10, x: 0, y: 5)
        
    }
}

#Preview {
    SavingsCard(user: User(name: "Erik", numberOfDosor: 2, pricePerDosa: 40.0, portionsPerDosa: 22, savingsGoal: 10000, startDate: Date())
    )
}
